...
class MemberController extends Controller
{
    ...
    'age_min'        => $request->get('age_from'),
    'age_max'        => $request->get('age_to'),
    ...
    public function show(Request $request, $id)
    {
        ...
        $member = [
            ...
            'is_own'     => ($myProfile['profile']['id'] ?? null) === ($d['profile_id'] ?? null) ? true : false,
            'basic' => [
                'name'            => $d['name'],
                'age'             => $d['age'],
                'dob'             => $d['dob'],
                ...
                'introduction'    => $d['introduction'] ?? $d['bio'],
            ],
            'horoscope' => [
                'dob'                => $d['dob'],
            ...
            'contact' => [
                'mobile'           => $d['mobile'] ?? $d['user']['phone'] ?? null,
    ...
    public function shortlist(Request $request, $id)
    {
        ...
        $msg = $response['message'] ?? 'Could not update shortlist. Please try again.';
    ...
    public function block(Request $request, $id)
    {
        ...
        $msg = $response['message'] ?? 'Could not block member. Please try again.';
        return response()->json(['message' => $msg], 422);
    }
}
