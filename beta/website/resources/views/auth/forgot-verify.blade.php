...
@section('title', __('ui.fp_verify_title'))
...
<div class="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4">
    <div class="max-w-md w-full">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 mt-2">{{ __('ui.fp_verify_subtitle') }}</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            <p class="text-sm text-gray-500 mb-6">{{ __('ui.fp_otp_sent_to') }} <strong>{{ session('fp_mobile') }}</strong>. Enter it below.</p>

            <form method="POST" action="{{ url('/forgot-password/verify-otp') }}">
                @csrf
                <div class="mb-5">
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('ui.fp_otp_label') }}</label>
                    <input type="text" name="otp" placeholder="{{ __('ui.fp_otp_placeholder') }}"
                ...
                <div class="mt-4 text-center text-sm text-gray-500">
                    <a href="{{ url('/forgot-password') }}" class="text-primary hover:underline">{{ __('ui.fp_back') }}</a>
                </div>
            </form>
        </div>
    </div>
