...
class AppModel extends CI_Model 
{
  ...
  public function getPlans()
   ...
   public function getPlan($plan_id)
   {
    ...
    function get_random_members($gender,$soveran,$ignored_ids,$ignored_by_ids)
    {
        $this->db->select('*');
        $this->db->from('member');
        $this->db->limit(12);
    ...
    function get_matched_members($gender,$partner_age,$partner_height,$partner_weight,$with_children_acceptables,$partner_any_disability,$partner_marital_status,$partner_education,$partner_body_type,$partner_DOSHAM,$partner_TYPE_OF_DOSHAM,$partner_Other_Dosham,$partner_Expectation,$partner_Other_Expectation,$soveran,$ignored_ids,$ignored_by_ids,$limit="",$start="")
    {
     ...
     function get_near_members($gender,$permanent_city,$soveran,$ignored_ids,$ignored_by_ids)
    {
        $this->db->select('*');
        $this->db->from('member');
        $this->db->limit(12);
        ...
        {
           $this->db->like('permanent_address','"permanent_city":"'.$permanent_city.'"','both');
        } 
        $this->db->where('is_blocked','no');
        $this->db->where('is_closed','no');
        $query=$this->db->get()->result(); 
  ...
  public function get_interest_pagination_datas($id,$limit,$start)
  {
    ...
    // public  function find_topping_pagination($idArr = array(),$gender="",$per_page="",$start=""){ 
    //  $query = $this->db->where_in("member_id", $idArr)->where('is_blocked','no')->where('is_closed','no')->where('delete_status',0)->where('gender',$gender)->limit($per_page,$start)->get("member");
    //      return $query->result();
    // }
  public  function find_topping_pagination($idArr = array(),$per_page="",$start="",$gender=""){ 
    ...
    public  function find_topping($idArr = array(),$gender="",$limit=""){ 
      $this->db->where_in("member_id", $idArr);
      $this->db->where('is_closed','no');
      $this->db->where('is_blocked','no');
      if(!empty($limit))
