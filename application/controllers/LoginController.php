...
// print_r($data);exit;
  if($member_approval == 'yes'){
      ...
      if($member_email_verification == 'on'){
          $this->db->update('member',array("phoneOtp"=>$rand_code),array('member_id'=>$insert_id));
          $this->db->update('member',array("email_verification_code"=>$rand_code),array('member_id'=>$insert_id));
          $this->LoginModel->member_email_verification('member', $data['email'], $rand_code);
          $this->sendMail();
      }
      $this->LoginModel->member_registration_email_to_admin($insert_id);

        //$code = rand(1000,9999);$this->db->update('member',array("phoneOtp"=>$code),array('member_id'=>$insert_id));
...
//$smsBody1="Dear ".$getData['first_name'].", your account has been created in Sri Sowdeswari Amman Narpani Mandram and your member ID : ".$getData1.". To access your profile kindly visit http://thirumanam.info/";
    ...
    public function sendMail(){
     ...
     $this->load->library('email');   
		...
		$config['smtp_host'] = 'mail.thirumanam.info'; 
		$config['smtp_user'] = 'noreply@thirumanam.info';
		$config['smtp_pass'] = 'Thiruman@2025#';
		...
		$this->email->from('noreply@thirumanam.info');
		$this->email->to('vgvimalraj90@gmail.com');
		$title='Thirumanam-OTP';
		$this->email->subject($title.' - one Time Password');
// 		$msg=$this->load->view('emails/order_completed',$datas, true);
$msg='This is test';
...
function checkPhoneOtp()
{
   ...
   //  print_r($id);exit;
    ...
    if($result)
    {
         ...
         $member_email_verification = get_settings_value('general_settings','member_email_verification');
         // print_r($member_email_verification);exit;
        $this->MetaModel->updateMemberDatas('member',array('member_id'=>$id),array("phoneVerifyStatus"=>1,"email_verification_status"=>1));
        // $this->db->update('member',array("phoneVerifyStatus"=>1),array('member_id'=>$id));
        if($member_approval == 'yes'){
          if ($result->status == "approved") {
            if($member_email_verification == 'on'){
